...
#define STOLEN_DSTACK_DATA_FREE(data) (marpa_free(data) ) 
...
#define marpa_free(p) free(p) 
#define marpa_new(type,count) ((type*) marpa_malloc(sizeof(type) *(count) ) ) 
#define marpa_renew(type,p,count) ((type*) marpa_realloc((p) ,sizeof(type) *(count) ) )  \

#define marpa_slice_alloc(size) marpa_malloc(size) 
#define marpa_slice_new(x) marpa_malloc(sizeof(x) ) 
#define marpa_slice_free(x,p) marpa_free(p) 
#define marpa_slice_free1(size,p) marpa_free(p)  \
